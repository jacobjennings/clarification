...
class ClarificationDataset final {
 public:
  explicit ClarificationDataset(
      ...
      producer_thread_([this] { PreloadBatchesLoop(); }) { // Launch producer thread
    if (batch_size <= 0) {
    ...
    std::cout << "ClarificationDataset initialized with sample_rate: " << sample_rate_ << ", sample_size_: " <<
 ...
 private:
  ...
  void PreloadBatches() {
    // Reset file index at the beginning of each preloading cycle
    file_idx = 0;
    ...
    while (current_batch_frames < num_preload_batches_ * batch_size_ && file_idx < sample_infos_.size()) {
        const auto &[path] = sample_infos_[file_idx];
        ...
        #pragma omp parallel num_threads(num_threads_)
        {
            ...
            #pragma omp critical
            {
                new_frames_vector.push_back(audio_samples);
    ...
    torch::Tensor new_frames_tensor = torch::cat(new_frames_vector, 0);

    // Split the new_frames_tensor into batches
    const int num_frames = new_frames_tensor.size(0);
    const int num_batches = (num_frames + batch_size_ - 1) / batch_size_;

    {
        std::lock_guard<std::mutex> lock(queue_mutex_);
        for (int i = 0; i < num_batches; ++i) {
            const int start_frame = i * batch_size_;
            const int end_frame = std::min(start_frame + batch_size_, num_frames);
            if (start_frame < end_frame) {
                torch::Tensor batch_tensor = new_frames_tensor.narrow(0, start_frame, end_frame - start_frame);
                preloaded_batches_.push(batch_tensor.view({-1, 2, sample_size}));
            }
        }
    }
  }
  ...
  void PreloadBatchesLoop() {
    while (!quit_) {
      ...
      // Sleep for a short duration to prevent busy-waiting when no new batches are needed
          ...
          while (avcodec_receive_frame(codec_context, frame) == 0) {
            ...
            if (is_planar) {
              ...
              for (int ch = 0; ch < num_channels; ++ch) {
                const auto *channel_data = reinterpret_cast<const float *>(frame->extended_data[ch]);
              ...
              const auto *interleaved_data = reinterpret_cast<const float *>(frame->extended_data[0]);
              for (int i = 0; i < num_samples; ++i) {
                for (int ch = 0; ch < num_channels; ++ch) {
